...
const getProfileFullDetails = createAsyncThunk(
  "profileFullDetails",
  async (payload, thunkApi) => {
    const profile_details = await getProfileDetails(
      thunkApi.getState().auth.token ?? ""
    );
    if (profile_details.status == 200) {
      thunkApi.dispatch(
        profileActions.setPartyFullData({
...
const saveDeliveryDetails = createAsyncThunk(
  "saveDeliveryDetails",
  async (payload, thunkApi) => {
    const update_response = await updateDeliveryAddressDetails(
      thunkApi.getState().auth.token ?? "",
      payload.name,
      payload.email,
      payload.shipto_houseno,
      payload.shipto_street,
      payload.shipto_locality,
      payload.shipto_city,
      payload.shipto_state,
      payload.shipto_pincode,
      payload.billing_same_as_shipping,
      payload.billing_houseno,
      payload.billing_street,
      payload.billing_locality,
      payload.billing_city,
      payload.billing_state,
      payload.billing_pincode,
      payload.buying_for_business,
      payload.gst_no,
  ...
  reducers: {
    ...
    setDeliveryAddress(state, action) {
      state.deliveryAddress = {
        ...
        party_address_line_1:
          action.payload.shipto_houseno +
          ", " +
          action.payload.shipto_street +
          ", " +
          action.payload.shipto_locality,
    ...
    setBillingAddress(state, action) {
      state.billingAddress = {
        ...state.billingAddress,
        party_address_line_1:
          action.payload.billing_houseno +
          ", " +
          action.payload.billing_street +
          ", " +
          action.payload.billing_locality,
        party_address_line_2: null,
