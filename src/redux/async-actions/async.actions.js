...
import { getOrderDetails, purchaseOrdersList } from "@/servers/lib-reown/lib";
import { refreshAndRepeat } from "@/servers/lib-reown/helpers";

export const getOrdersList = createAsyncThunk(
  "getOrdersList",
  async (payload, thunkApi) => {
    let resp = await purchaseOrdersList(
...
export const getOrderData = createAsyncThunk(
  "orderDetails",
  async (payload, thunkApi) => {
    let resp = await getOrderDetails(
      thunkApi.getState().auth.token ?? "",
      payload
    );
    if (resp.status == 200) {
