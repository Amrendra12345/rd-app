...
const removeCart = createAsyncThunk("removeCart", async (payload, thunkApi) => {
  if (!checkAuthData(thunkApi, authActions.openSidebar)) return null;
...
const syncCartData = createAsyncThunk(
  "syncCartData",
...
const applyCouponAction = createAsyncThunk(
  "applyCoupon",
  async (payload, thunkApi) => {
    if (!checkAuthData(thunkApi, authActions.openSidebar)) return null;
...
const revokeCouponAction = createAsyncThunk(
  "applyCoupon",
  async (payload, thunkApi) => {
    if (!checkAuthData(thunkApi, authActions.openSidebar)) {
      thunkApi.dispatch(cartActions.setPendingAction(true));
    ...
    if (resp.status == 200) {
      ...
      const repeat_res = await refreshAndRepeat(
        ...
        resp,
        payload,
        cartActions.revokeCouponAction,
        authActions.refreshTokens,
        authActions.logout,
        authActions.openSidebar
      );
