...
const signupOtp = createAsyncThunk(
  "signupOtp",
...
const verifySignup = createAsyncThunk(
  "verifySignup",
  async ({ mobile_number, otp }, thukApi) => {
 ...
 */
const checkAndLoadInitialAuth = createAsyncThunk(
  "loadInitialAuth",
      ...
      .addCase(verifyLogin.fulfilled, (state, action) => {
        if (action.payload.status == 200) {
          if (action.payload.data.profile) {
            ...
            state.errorMessage =
              action.payload.data.status_message ?? "Some error occurred";
          }
        } else {
          state.errorMessage = action.payload.message;
        }
